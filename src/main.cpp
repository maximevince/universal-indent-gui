...
//				std::cerr << std::endl << "The parameter "<< i << " is an invalid finder parameter. Parameter was " << argv[i] << std::endl;
    ...
    //XXX FIXME QTextCodec::setCodecForCStrings( QTextCodec::codecForName("UTF-8") );
    ...
    QMessageLogContext dummyCtx;
    TSLogger::messageHandler( TSLoggerInfoMsg, dummyCtx, QString("Starting UiGUI Version %1 %2").arg(PROGRAM_VERSION_STRING).arg(PROGRAM_REVISION).toLatin1() );
    TSLogger::messageHandler( TSLoggerInfoMsg, dummyCtx, QString("Running on %1").arg(UiGuiSystemInfo::getOperatingSystem()).toLatin1() );
